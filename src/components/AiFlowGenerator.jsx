...
const FlexibleFlowGenerator = () => {
  ...
  const [headerTypes, setHeaderTypes] = useState({}); // {templateIndex: 'none'|'text'|'image'}
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        const templates = response.data.missing_templates || [];
        setAllTemplates(templates);
        setFlowPlan(response.data.flow_plan);
        
        // Initialize header types based on AI suggestion
        const initialHeaderTypes = {};
        templates.forEach((template, index) => {
          if (template.template_requirements?.needs_media) {
            initialHeaderTypes[index] = 'image';
          } else {
            initialHeaderTypes[index] = 'none';
          }
        });
  ...
  // NEW: Handle header type change (none, text, image)
  const handleHeaderTypeChange = (templateIndex, newType) => {
    setHeaderTypes(prev => ({
      ...prev,
      [templateIndex]: newType
    }));

    // Update template requirements
    setAllTemplates(prev => {
      const updated = [...prev];
      if (!updated[templateIndex].template_requirements) {
        updated[templateIndex].template_requirements = {};
      }
      
      updated[templateIndex].template_requirements.needs_media = (newType === 'image');
      
      return updated;
    });

    // Clear image if switching away from image type
    if (newType !== 'image' && templateImages[templateIndex]) {
  ...
  const handleRemoveImage = (templateIndex) => {
    setTemplateImages(prev => {
      const updated = {...prev};
      delete updated[templateIndex];
      return updated;
  ...
  const handleSubmitToMeta = async () => {
    ...
    // Validate: if header type is image, file must be uploaded
    for (const index of selectedTemplates) {
      if (headerTypes[index] === 'image' && !templateImages[index]) {
        alert(`Template ${allTemplates[index].suggested_name} साठी image अपलोड करा / Please upload image for template ${allTemplates[index].suggested_name}`);
        return;
      }
    }
    ...
    for (const index of selectedTemplates) {
      ...
      try {
        ...
        alert(`Template ${template.suggested_name} सबमिट करण्यात अयशस्वी: ${error.response?.data?.message || error.message}`);
      }
    }
  ...
  const buildComponents = (requirements, templateIndex) => {
    ...
    // Header based on header type
    const currentHeaderType = headerTypes[templateIndex];
    
    if (currentHeaderType === 'image' && templateImages[templateIndex]) {
      components.push({
        type: 'HEADER',
        format: 'IMAGE'
      });
    } else if (currentHeaderType === 'text') {
      components.push({
        type: 'HEADER',
        format: 'TEXT',
        text: '⭐ स्वागत आहे / Welcome'
      });
    }
    // If 'none', don't add header component
        ...
        {allTemplates.map((template, index) => (
          <div key={index} className={`template-card ${selectedTemplates.includes(index) ? 'selected' : ''}`}>
            ...
            <div className="template-body">
              ...
              {/* NEW: Header Type Selector with Buttons */}
              <div className="field-group header-type-section">
                <label><strong>Header Type:</strong></label>
                <div className="header-type-buttons">
                  <button
                    type="button"
                    className={`header-type-btn ${(headerTypes[index] || 'none') === 'none' ? 'active' : ''}`}
                    onClick={() => handleHeaderTypeChange(index, 'none')}
                  >
                    🚫 No Header
                  </button>
                  <button
                    type="button"
                    className={`header-type-btn ${(headerTypes[index] || 'none') === 'text' ? 'active' : ''}`}
                    onClick={() => handleHeaderTypeChange(index, 'text')}
                  >
                    📝 Text Header
                  </button>
                  <button
                    type="button"
                    className={`header-type-btn ${(headerTypes[index] || 'none') === 'image' ? 'active' : ''}`}
                    onClick={() => handleHeaderTypeChange(index, 'image')}
                  >
                    🖼️ Image Header
                  </button>
                </div>
              ...
              {/* Show image upload section if image header selected */}
              {(headerTypes[index] || 'none') === 'image' && (
                  ...
                  {!templateImages[index] ? (
                    <label htmlFor={`file-${index}`} className="upload-label">
                      <div className="upload-placeholder">
                        ...
                        <p><strong>Image अपलोड करा / Upload Image</strong></p>
                    ...
                    <div className="image-preview-box" >
                      ...
                      <div className="image-info">
                        <p><strong>{templateImages[index].name}</strong></p>
                        <small>{(templateImages[index].size / 1024).toFixed(2)} KB</small>
                      </div>
                      <button
                        type="button"
              ...
              <div className="buttons-edit-section">
                <label><strong>Buttons (max 3):</strong></label>
